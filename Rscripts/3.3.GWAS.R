...
#save(G_grape,M_grape,mapping,Q,phenoT, file = "data/GWAS_input_v2.RData")
...
for (k in 1:length(phenoT)){
  ...
  for(i in 1:length(models)){
    for(j in 1:length(colnames(data)[-1])){ #
       s <- getwd()
       f <- paste0(getwd(), paste0("/GWAS_output/"))
      ...
      gapit_output[[set.names[k]]][[models[i]]][[y2]] <- mod
      setwd(s)
    }
  }
}

#save(gapit_output, file = "data/GWAS_output/GWAS_object.RData")
