...
setwd("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE/ProcessedVCFs/SNPGENOTASSELOUTPUT/")

# Read the PCs file
pcs <- read.table("6_tassel_analysis/pca_filtered1.txt", header = TRUE, sep = "\t", as.is = TRUE, skip=2)
pcs2 <- read.table("6_tassel_analysis/pca_filtered2.txt", header = FALSE, sep = "\t", as.is = TRUE, skip=2)
pcs3 <- read.table("6_tassel_analysis/pca_filtered3.txt", header = FALSE, sep = "\t", as.is = TRUE, skip=2)
...
PCplot<-ggplot(filtered_pcs_nonduplicated, aes(x=PC1, y=PC2, color=PC3)) + geom_point() + theme_minimal()+
    geom_text(aes(label=Isolate),hjust=0, vjust=0, size=1)+
    theme(legend.position = "none")


# save the plot
ggsave("PCplot.png", PCplot, width = 10, height = 10, units = "cm")
filtered_pcs_nonduplicated




# load the phenotypic data from "/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE/PhenoData/raw_phenotypes.csv"

# read the file
phenotypes<-read.csv("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE/PhenoData/raw_phenotypes.csv")
head(phenotypes)
...
for (i in 1:nrow(phenotypes)){
  for (j in 1:nrow(filtered_pcs_nonduplicated)){
    if (grepl(filtered_pcs_nonduplicated$Isolate[j], phenotypes$Picture[i])){
...
colnames(phenotypes)
# boxplot of phenotypes$PLACL wrt phenotypes$Isolate
ggplot(phenotypes, aes(x=Isolate, y=PLACL)) + geom_boxplot() + theme_minimal()+
...
PhenoGWAS<-phenotypes[,c("Isolate", "REP", "leave_id","WheatGeno", "Year", "PLACL", "pycnidiaPerCm2Leaf",
...
PhenoGWAS[1:5,]
# only leaf 2
PhenoGWAS<-PhenoGWAS[PhenoGWAS$leave_id=="2",]
...
intersectnames<-intersect(colnames(kinship), colnames(geno))
...
traits <- c("leafArea", "PLACL", "pycnidiaPerCm2Leaf", "pycnidiaPerCm2Lesion","meanPycnidiaArea")
...
plot_gwas <- function(GWASOut, GenoMat, trait, annotatePval = 0.005, annotateTop = FALSE) {
  ...
  manhattan(GWASData, chr="CHR", bp="BP", snp="SNP", p="P", main=paste("Manhattan Plot for", trait),
...
plot_qq <- function(GWASOut, trait) {
  ...
  # Generate QQ plot
  qq(pvals, main=paste("QQ Plot for", trait))
}
...
run_gwas<-function(trait, PhenoGWAS, GenoMat, SNPMAP, kinship, dataName, annotatePval = 0.005, annotateTop = FALSE) {
  ...
  # LMER model
  model <- lmer(as.formula(paste(trait, "~ REP + leave_id + Year + WheatGeno + (1|line)", sep="")), data=PhenoGWAS)
...
genoMat[is.na(genoMat)]<-mean(genoMat, na.rm=TRUE)
...
for(trait in traits) {
  run_gwas(trait, PhenoGWAS, geno, SNPMAP, kinship, "GenoMat_PC3",annotatePval = 0.1/nrow(geno), annotateTop = FALSE)
}
...
# Main loop to run GWAS for each trait
for(trait in traits) {
  run_gwas(trait, PhenoGWAS, GenoMAGWAS, SNPMAPMA, kinship, "GenoMA_PC3",annotatePval = 0.1/nrow(GenoMAGWAS), annotateTop = FALSE)
}
...
# Main loop to run GWAS for each trait
for(trait in traits) {
  run_gwas(trait, PhenoGWAS, GenoPCGWAS, SNPMAPPC, kinship, "GenoPC_PC3",annotatePval = 0.1/nrow(genoPCs), annotateTop = FALSE)
}
...
# Assuming `kinship_matrix` is your kinship matrix
# You may need to install the "gplots" package if you haven't already
#install.packages("gplots")
library(gplots)

# Plot the kinship matrix as a heatmap
# Assuming `kinship_matrix` is your kinship matrix
# You may need to install the "gplots" package if you haven't already
install.packages("gplots")
library(gplots)
...
# Increase contrast for off-diagonals
color_scale <- colorpanel(100, "blue", "white", "red")

# Plot the kinship matrix as a heatmap
# save as png
png("kinship.png")
heatmap(kinship_matrix, 
        symm = TRUE,         # Symmetric matrix
        scale = "none",      # No scaling
        col = color_scale,   # Color scheme for heatmap
        margins = c(5, 10),  # Adjust margins for better visualization
        main = "Kinship Matrix",  # Title of the plot
        xlab = "Individuals",     # Label for x-axis
        ylab = "Individuals")     # Label for y-axis
