...
# setwd("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE2/Rscripts/GWASforPilarTraitsMarch27th")
# tmp<-capture.output({
# modK <- GAPIT(Y=BLUPS,
#                 GD=GenoMat13,
#                 GM=mapping,
...
if (!file.exists("GWASforPilarTraitsMarch27thThinnedSnps")){
  dir.create("GWASforPilarTraitsMarch27thThinnedSnps", showWarnings = FALSE)
}

setwd("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE2/Rscripts/GWASforPilarTraitsMarch27thThinnedSnps")
...
mapping_thinned <- mapping %>% 
  ...
  thin_snps(distance = 500) %>%as.data.frame() # Thinning within each 100 kb
...
if (!file.exists("GWASforPilarTraitsMarch27thThinnedSnpsWithCorwoOutliers")){
  dir.create("GWASforPilarTraitsMarch27thThinnedSnpsWithCorwoOutliers", showWarnings = FALSE)
}

setwd("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE2/Rscripts/GWASforPilarTraitsMarch27thThinnedSnpsWithCorwoOutliers")
...
mapping_highest_corr <- find_highest_corr_snp(mapping13, GenoMat13, 100) # Adjust the distance as needed
...
dim(mapping13_thinned)
sum(is.na(BLUPS))
###Calculate the PCA for the thinned data
svdG<-svd(scale(GenoMat13_thinned[,-1], scale=FALSE, center=TRUE), nu=5,nv=5)
pcs<-scale(GenoMat13_thinned[,-1], scale=FALSE, center=TRUE)%*%svdG$v
pcs<-cbind(data.frame(Taxa=GenoMat13_thinned$Taxa), pcs)
colnames(pcs)[2:6]<-paste("PC",1:5, sep="")
# plot the first two PCs
p <- ggplot(pcs, aes(x=PC1, y=PC2, label=Taxa)) +
  geom_point() +
  geom_text_repel(aes(label=Taxa), size=1, 
                  box.padding = .1, 
                  point.padding = 0.1,
                  segment.color = 'grey50', max.overlaps = 120) +
  xlim(min(pcs$PC1) - 5, max(pcs$PC1) + 5) + # Adjust xlim
...
# subset PCs for the isolates that have PC1 > -50
pcsOut<-pcs[pcs$PC1<=-50,]
pcsOut$Taxa
pcs<-pcs[pcs$PC1>-50,]
pcs$Taxa

# subset the BLUPs, GenoMat13_thinned 
BLUPS<-BLUPS[BLUPS$Taxa%in%pcs$Taxa,]
GenoMat13_thinned<-GenoMat13_thinned[GenoMat13_thinned$Taxa%in%pcs$Taxa,]
