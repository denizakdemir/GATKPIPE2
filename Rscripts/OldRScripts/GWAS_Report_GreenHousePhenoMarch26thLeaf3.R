...
# select only leave_id==3
phenotypes<-phenotypes[phenotypes$leave_id==3,]
...
intersect(colnames(phenotypes), c(c("Isolate", "REP","WheatGeno", "Year", "PLACL", "pycnidiaPerCm2Leaf",
"pycnidiaPerCm2Lesion", "necrosisArea", "pycnidiaCount", "leafArea", "meanPycnidiaArea","pycnidiaGreyValue")))
PhenoGWAS<-phenotypes[,c(c("Isolate", "REP","WheatGeno", "Year", "PLACL", "pycnidiaPerCm2Leaf",
"pycnidiaPerCm2Lesion", "necrosisArea", "pycnidiaCount", "leafArea", "meanPycnidiaArea","pycnidiaGreyValue"))]
...
# These are the fixed effects:  "REP"                           
# [4] "WheatGeno"            "Year" 
...
library(lme4)
ModelLMEPLACL<-lmer(PLACL~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
BLUPPLACL<-ranef(ModelLMEPLACL)$line[,1]
ModelLMEpycnidiaPerCm2Leaf<-lmer(pycnidiaPerCm2Leaf~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
BLUPpycnidiaPerCm2Leaf<-ranef(ModelLMEpycnidiaPerCm2Leaf)$line[,1]
ModelLMEpycnidiaPerCm2Lesion<-lmer(pycnidiaPerCm2Lesion~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
BLUPpycnidiaPerCm2Lesion<-ranef(ModelLMEpycnidiaPerCm2Lesion)$line[,1]
ModelLMEnecrosisArea<-lmer(necrosisArea~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
BLUPnecrosisArea<-ranef(ModelLMEnecrosisArea)$line[,1]
ModelLMEpycnidiaCount<-lmer(pycnidiaCount~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
BLUPpycnidiaCount<-ranef(ModelLMEpycnidiaCount)$line[,1]
ModelLMEleafArea<-lmer(leafArea~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
BLUPleafArea<-ranef(ModelLMEleafArea)$line[,1]
ModelLMEmeanPycnidiaArea<-lmer(meanPycnidiaArea~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
BLUPmeanPycnidiaArea<-ranef(ModelLMEmeanPycnidiaArea)$line[,1]
ModelLMEpycnidiaGreyValue<-lmer(pycnidiaGreyValue~REP+WheatGeno+Year+(1|line), data=PhenoGWAS)
...
# make a folder to save the output files "GWASforPilarTraitsMarch27thLeaf3"
if (!file.exists("GWASforGreenHouseTraitsMarch27thLeaf3")){
dir.create("GWASforGreenHouseTraitsMarch27thLeaf3", showWarnings = FALSE)
}


setwd("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE2/Rscripts/GWASforGreenHouseTraitsMarch27thLeaf3")
tmp<-capture.output({
modK <- GAPIT(Y=BLUPS,
                GD=GenoMat13,
                GM=mapping,
...
if (!file.exists("GWASforGreenHouseTraitsMarch27thThinnedSnpsLeaf3")){
  dir.create("GWASforGreenHouseTraitsMarch27thThinnedSnpsLeaf3", showWarnings = FALSE)
}

setwd("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE2/Rscripts/GWASforGreenHouseTraitsMarch27thThinnedSnpsLeaf3")
...
if (!file.exists("GWASforGreenHouseTraitsMarch27thThinnedSnpsWithCorLeaf3")){
  dir.create("GWASforGreenHouseTraitsMarch27thThinnedSnpsWithCorLeaf3", showWarnings = FALSE)
}

setwd("/Users/denizakdemir/Library/CloudStorage/GoogleDrive-deniz.akdemir.work@gmail.com/.shortcut-targets-by-id/1n5uwNs72GdTeZxmWZKLfFkgE0FIjPxVG/Akdemir_Deniz/github/GATKPIPE2/Rscripts/GWASforGreenHouseTraitsMarch27thThinnedSnpsWithCorLeaf3")
