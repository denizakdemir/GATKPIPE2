...
head(phenotypes)
########remove based on Removal
table(phenotypes$Removal, useNA = "ifany")
dim(phenotypes)
# Astringent      Loose 
#       355        379
phenotypesSR<-phenotypes[phenotypes$Removal!="Astringent",]
dim(phenotypesSR)
PhenoGWAS<-phenotypesSR[,c("Isolate", "BReplicate", "TReplicate", "Endophyte","Growth_endophyte", "Area_inhibition", "Radius_inhibition", "Ratio_inhibition", "Percentage_inhibition")]
...
new_dir <- "DataforPillarApr25_filtered_loose"
...
load("DataforPillarApr25_filtered_loose/DataforPillar.RData")
...
save(PhenoGWASR3E3, GenoMat13_thinned,mapping13_thinned,file="DataforPillarApr25_filtered_loose/DataforPillarGWAS.RData")

# calculate the Kmat
Kmat<-rrBLUP::A.mat(GenoMat13_thinned[, -1]-1, min.MAF = .1, max.missing = .3)

# save Kmat, GenoMat13_thinned, mapping13_thinned 
save(Kmat, GenoMat13_thinned, mapping13_thinned, file = "DataforPillarApr25_filtered_loose/GenoDataSeptoria100.RData")
