# Load necessary data
load("Pheno_geno_Map_GreenHouseTraits.RData")
...
#######################################
# Split phenotypes based on 'leave_id' criteria
phenotypesLeave2 <- phenotypes[phenotypes$leave_id == "2", ]
PhenoGWASLeave2 <- phenotypesLeave2[, c("Isolate", "REP", "WheatGeno", "Year", "PLACL", "pycnidiaPerCm2Leaf",
                                        "pycnidiaPerCm2Lesion", "necrosisArea", "pycnidiaCount", "leafArea",
                                        "meanPycnidiaArea", "pycnidiaGreyValue")]

phenotypesLeave3 <- phenotypes[phenotypes$leave_id == "3", ]
PhenoGWASLeave3 <- phenotypesLeave3[, c("Isolate", "REP", "WheatGeno", "Year", "PLACL", "pycnidiaPerCm2Leaf",
                                        "pycnidiaPerCm2Lesion", "necrosisArea", "pycnidiaCount", "leafArea",
                                        "meanPycnidiaArea", "pycnidiaGreyValue")]

# Rename column 'Isolate' to 'line'
colnames(phenotypesLeave2)[1] <- "line"
colnames(phenotypesLeave3)[1] <- "line"

# Convert specific columns to factors
factor_columns <- c("line", "REP", "WheatGeno", "Year")
phenotypesLeave2[factor_columns] <- lapply(phenotypesLeave2[factor_columns], as.factor)
phenotypesLeave3[factor_columns] <- lapply(phenotypesLeave3[factor_columns], as.factor)


PhenotypeDataList <- list(
  phenotypesLeave2 = phenotypesLeave2,
  phenotypesLeave3 = phenotypesLeave3
)
...
calculate_blups <- function(data, trait) {
  model <- lmer(formula(paste(trait, "~ REP+WheatGeno+Year + (1|line)")), data = data)
...
run_analysis <- function(pheno_data) {
  ...
  traits <- c( "PLACL", "pycnidiaPerCm2Leaf",
               "pycnidiaPerCm2Lesion", "necrosisArea", "pycnidiaCount", "leafArea",
               "meanPycnidiaArea", "pycnidiaGreyValue")
...
new_dir <- "GWAS_resultsForGreenHouseTraits"
