...
COMBINED_OUTPUT_FILE="${OUTPUT_DIR}/combined_sorted_md_sequences.fasta"
...
# Initialize combined output file for sorted_md files
> ${COMBINED_OUTPUT_FILE}  # Clear the file if it already exists

# Iterate over individual VCF files and filter only files with 'sorted_md'
for VCF_FILE in ${VCF_DIR}/*sorted_md*.vcf.gz; do
    ...
    for REGION in "${REGIONS[@]}"; do
        ...
        # Append the consensus sequence to the combined output file
        cat temp_header.fasta >> ${COMBINED_OUTPUT_FILE}
...
echo "Consensus sequence compilation and combination for 'sorted_md' strains completed."
